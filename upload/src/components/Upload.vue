  ...
  export default {
      ...
      methods: {
          ...
          upload: function() {
              ...
              let it = 0
              let self = this
              axios.get('http://localhost:8081/api/apikey', {headers: {'Authorization': 'token='+this.apiKey}})
                .then((response) =>{
                  this.uploaded = true
                  for (let i = 0; i < total; i++) {
                      ...
                      axios.post('https://api.cloudinary.com/v1_1/'+response.data.cloudName+'/upload', formData)
                          .then((data2) => {
                              ...
                              axios.post('http://localhost:8081/api/images/add',{headers: {'Authorization': 'token='+this.apiKey}}, {data: pic})
                          })
                          .catch((error) => {
                              console.log(error)
                              it++
